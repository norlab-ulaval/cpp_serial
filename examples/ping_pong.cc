// ping_pong.cc: Continuously sends a message and prints received messages.
// Usage: ./ping_pong <device> <baudrate> <message> --interval-ms <interval>
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "[ping_pong] Started on " << port << " at " << baud << " baud.\n";
    while (true) {
        // Send message
        ser.write(message);
        ...
        if (!result.empty()) {
            std::cout << "[ping_pong] Received: " << result << std::endl;
        }
